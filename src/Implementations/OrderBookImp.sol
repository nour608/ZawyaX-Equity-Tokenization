...
abstract contract OrderBook is DataTypes {
    ...
    event TradeExecuted(
        uint256 indexed tradeId, uint256 indexed projectId, address buyer, address seller, uint256 shares, uint256 price
    );
    ...
    function cancelOrder(uint256 orderId) public virtual {
        ...
        require(
            order.status == DataTypes.OrderStatus.ACTIVE || order.status == DataTypes.OrderStatus.PARTIALLY_FILLED,
     ...
     */
    function matchOrdersForProject(uint256 projectId, uint256 tradingFeeRate)
        public
        returns (uint256 tradesExecuted, uint256 feeAmount)
    {
        ...
        while (buyOrderIds.length > 0 && sellOrderIds.length > 0) {
            ...
            if (
                buyOrder.pricePerShare >= sellOrder.pricePerShare && buyOrder.status == DataTypes.OrderStatus.ACTIVE
                    && sellOrder.status == DataTypes.OrderStatus.ACTIVE
            ) {
                feeAmount = _executeTrade(buyOrderIds, sellOrderIds, projectId, bestBuyId, bestSellId, tradingFeeRate);
                tradesExecuted++;
            } else {
     ...
     */
    function getMarketPrice(uint256 projectId) external view virtual returns (uint256) {
    ...
    function getUserOrders(address user, uint256 projectId)
        ...
        view
        virtual
        returns (DataTypes.Order[] memory userProjectOrders)
    {
        ...
        for (uint256 i = 0; i < orderIds.length; i++) {
            if (
                orders[orderIds[i]].projectId == projectId
                    && (
    ...
    function _insertBuyOrder(uint256 orderId, uint256[] storage buyOrderIds) internal {
    ...
    function _insertSellOrder(uint256 orderId, uint256[] storage sellOrderIds) internal {
        DataTypes.Order storage newOrder = orders[orderId];
    ...
    function _executeTrade(
        ...
        uint256 tradedShares =
            buyOrder.sharesRemaining < sellOrder.sharesRemaining ? buyOrder.sharesRemaining : sellOrder.sharesRemaining;
    ...
    function _updateMarketStats(DataTypes.MarketStats storage stats, uint256 price, uint256 volume) internal {
